...
#'
#' @param wide_file_windows_trimmed A data frame in wide format.
...
wide_to_long_trimmed_windows <- function(wide_file_windows_trimmed, start_prefix = "start", end_prefix = "end") {
  ...
  intermediate_long_file_windows_trimmed <- melt(wide_file_windows_trimmed, id.vars = id_vars, measures.vars = c(windows_start, windows_end))

  # Splitting the variable column to identify whether the row has a start or end value of a window
  intermediate_long_file_windows_trimmed[, c("type", "WindowName") := tstrsplit(sub("_", " ", variable), " ")][, variable := NULL]

  # Casting to long format but only the date values
  long_file_windows_trimmed <- dcast(intermediate_long_file_windows_trimmed, ... ~ factor(type, levels = c(start_prefix, end_prefix)), value.var = "value")
