...
In this step, we will identify the events that occurred within each window after they have been **altered** or **cleaned**. This process involves two main steps:

1. **Transform Wide to Long Format**: First, we need to transform the wide-formatted data.table resulting from the `clean_windows()` function to a long-formatted data.table. This is done using the `wide_to_long_trimmed_windows()` function.
2. **Select Dates Within Windows**: Next, we use the `select_dates_window()` function to identify all (or, if specified, only the first) records that fall within a specified window.

### Step 3.1: Transform Wide to Long Format `wide_to_long_trimmed_windows()`

This function takes `windows_trimmed` in a wide format and transforms it into a long format, making it compatible with the `select_dates_window` function.

**Parameters:**
- `wide_file_windows_trimmed`: A data table in wide format.
- `start_prefix`: A string that defines the prefix for the start of a window. Default is "start".
- `end_prefix`: A string that defines the prefix for the end of a window. Default is "end".
...
wide_file_windows_trimmed <- data.table::data.table(
  person_id = c(1, 2),
  start_window1 = as.Date(c('2021-01-01', '2021-01-01')),
  end_window1 = as.Date(c('2021-01-02', '2021-01-03')),
  start_window2 = as.Date(c('2021-02-01', '2021-02-01')),
  end_window2 = as.Date(c('2021-02-02', '2021-02-03'))
)

# Function call
long_format <- wide_to_long_trimmed_windows(
  wide_file_windows_trimmed = wide_file_windows_trimmed,
  start_prefix = "start",
  end_prefix = "end"
)
...
### Step 3.2: Select Dates Window `select_dates_window()`
...
- `start_window_date_col_name`: The start date column name in the scri_trimmed. Default is "start".
- `end_window_date_col_name`: The end date column name in the scri_trimmed. Default is "end".
...
records_table <- data.table::data.table(
  person_id = c(1, 1, 2, 2),
  date = as.Date(c('2021-01-01', '2021-01-02', '2021-01-01', '2021-01-03')),
  value = c(10, 20, 30, 40)
)

scri_trimmed <- data.table::data.table(
  person_id = c(1, 2),
  start = as.Date(c('2021-01-01', '2021-01-01')),
  end = as.Date(c('2021-01-02', '2021-01-03'))
)

# Function call
result <- select_dates_window(
