...
In this step, we use the `construct_windows()` function to calculate the start and end dates of different analysis windows. Let's start by grabbing the relevant windows from our example metadata file
...
The `construct_windows()` returns start and end dates for each of the windows under consideration, anchored to the relevant reference date. The output by default is wide format. The output object also inherits all columns from the input `studypopulation`
...
The function `wrangle_window()` takes as input a wide-format dataset following the same format as that created by `construct_windows(..., output_format = "wide")`. In addition, the function takes the names of different date columns in the input dataset which are used for censoring. 
