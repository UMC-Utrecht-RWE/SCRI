test_that("wrangle_window retruns expected shape",{
  library(data.table)
  sp_obj_df <- data.table(id = c(1,2),
  ...
  wrangle_window(sp_obj_df, censoring_dates = c("general_end_date","death_date"))
   

  sp_obj <- data.table::as.data.table(sp_obj_df)
  testout1 <- suppressMessages(wrangle_window(sp_obj,
